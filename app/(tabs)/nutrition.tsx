...
export default function NutritionScreen() {
  ...
  const calculateStrategy = (user: UserProfile, analysis: any) => {
    ...
    if (heightM > 0 && weightKg > 0) {
      ...
      if (bmi < 18.5) { bmiStatus = i18n.t('nutrition.bmi.underweight'); bmiColor = "#3498db"; }
      else if (bmi < 25) { bmiStatus = i18n.t('nutrition.bmi.normal'); bmiColor = "#2ecc71"; }
      else if (bmi < 30) { bmiStatus = i18n.t('nutrition.bmi.overweight'); bmiColor = "#f1c40f"; }
      else { bmiStatus = i18n.t('nutrition.bmi.obese'); bmiColor = "#e74c3c"; }
    }
    ...
    const activityMultipliers: { [key: string]: number } = {
      ...
      'Leggermente attivo': 1.375, 'Lightly active': 1.375,
      'Moderatamente attivo': 1.55, 'Moderately active': 1.55,
      'Molto attivo': 1.725, 'Very active': 1.725,
      'Estremo': 1.9, 'Extreme': 1.9
    };
    const multiplier = activityMultipliers[user.activityLevel] || 1.2;
    const tdee = bmr * multiplier;

    let goalCalories = tdee;
    let goalType = i18n.t('goals.maintenance');

    if (bmi > 25) {
      goalCalories = tdee - 500;
      goalType = i18n.t('goals.weightLoss');
    } else if (bmi < 18.5) {
      goalCalories = tdee + 300;
      goalType = i18n.t('goals.weightGain');
    }
    ...
    const hasDiabetes = user.conditions.includes("Diabete") || user.conditions.includes("Diabetes");
    if (hasDiabetes) {
      carbsPct = 30;
      proteinPct = 35;
      fatsPct = 35;
      goalType += ` (${i18n.t('goals.lowCarb')})`;
    }
